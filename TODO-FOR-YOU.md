...
## ① 重設 Supabase 資料庫密碼 —— 真的要上線前必做

> **目前擱置。** 這個專案沒有真的對外開放，你決定專案結束再處理。
> 但只要哪天要讓外人使用，這一項必須先做完 ——
...
```json
{
  ...
  "database": "postgres",        // sqlite = 設定有問題，資料會在重啟時消失
  "redis": true,                 // false = 登出後 token 不會立即失效
  "storage": "supabase",         // database = 圖片還存在資料庫裡
  "storageLastError": null,      // 有值 = 物件儲存寫入失敗，正在落回資料庫
  "ai": "huggingface",           // local = 沒接上模型，在跑本機樣板
  "aiLastError": null            // 有值 = 最後一次模型呼叫失敗的代碼
}
```

**兩個 `LastError` 欄位是刻意加的。** 這兩處都有備援：物件儲存寫不進去就
落回資料庫、AI 呼叫失敗就退回本機樣板。備援讓功能不中斷，但也讓故障
變得看不出來 —— AI 曾經整整壞了一段時間都沒人發現，就是因為備援太安靜。
現在故障會出現在這裡。
...
- **Web Push** —— 瀏覽器整個關掉也收得到。金鑰程式自己產生，你不用設定
- **服務保溫** —— GitHub Actions 排程，見第②節
- **登出後 token 立即失效**、**改密碼登出所有裝置**
- 圖片內容定址與儲存層抽換（資料庫 ↔ 物件儲存）
- AI 診斷端點，以及健康檢查上的兩個故障代碼
- 所有程式碼層面的安全防護
